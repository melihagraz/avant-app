...
import { fetchMatchInsights, type HighlightCard } from '../lib/matchInsights';
...
export default function MatchRevealScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    ...
    // Insights are generated by the edge function shortly after the match
    // is created, so they may not yet be available on this screen's first
    // render. Try once; if empty, retry after a short delay.
    let cancelled = false;
    const load = async (attempt = 0) => {
      const data = await fetchMatchInsights(match.id);
      if (cancelled) return;
  ...
  const openReel = () => {
    hapticLight();
    trackEvent('highlight_reel_open', { match_id: match.id });
    setReelOpen(true);
  };
    ...
    <View style={s.wrap}>
      ...
      <View style={s.content}>
        ...
        <Animated.View style={[s.buttonsWrap, buttonsStyle]}>
          {highlights.length > 0 && (
            <TouchableOpacity onPress={openReel} style={s.primaryBtn} activeOpacity={0.85}>
              <LinearGradient
                colors={colors.accentGradient as any}
                start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                style={s.primaryBtnGrad}
              >
                <Text style={s.primaryBtnTxt}>✨ {t('matchReveal.viewHighlights')}</Text>
              </LinearGradient>
            </TouchableOpacity>
          )}

          <TouchableOpacity onPress={closeAndChat} style={highlights.length > 0 ? s.secondaryBtn : s.primaryBtn} activeOpacity={0.85}>
            {highlights.length > 0 ? (
              <Text style={s.secondaryBtnTxt}>{t('matchReveal.sayHi')}</Text>
            ) : (
              <LinearGradient
                colors={colors.accentGradientAlt as any}
                start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
          ...
          <TouchableOpacity onPress={closeAndDetail} style={s.tertiaryBtn} activeOpacity={0.85}>
            <Text style={s.tertiaryBtnTxt}>{t('matchReveal.viewProfile')}</Text>
          </TouchableOpacity>
        </Animated.View>
      </View>
